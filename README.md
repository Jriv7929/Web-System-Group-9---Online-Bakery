...
To ensure clear visibility into our progress and to manage risks effectively, we adopted a structured planning approach.

### Kanban/Checklist
| Status | Task Description | Risk/Backup Plan (if applicable) |
| :--- | :--- | :--- |
| **✅ Done** | Static pages (Home, About, Contact, Products) design and implementation. | N/A |
| **✅ Done** | Basic Shopping Cart structure (HTML/CSS). | N/A |
| **✅ Done** | Advanced Shopping Cart functionality with `localStorage` persistence and quantity controls. | N/A |
| **✅ Done** | Promo Code logic and validation. | N/A |
| **✅ Done** | Accessibility and keyboard navigation improvements. | N/A |
| **🚧 In Progress** | Implementing a functional **Contact Us** page using a form submission service. | Backup Plan: Use a simple email link until a backend solution is ready. |
| **💡 Planned** | Full User Account system (Login/Register with persistence). | N/A |
| **💡 Planned** | Backend integration for order placement and data storage. | Backup Plan: If a full backend is unfeasible, focus on a robust "order summary" that users can copy/paste. |
...
- **Fixed critical promo code bugs and the newsletter subscribe function.**
...
The following issues reported by the QA team have been **fixed** and verified in the latest commit:

* **Promo Code Removal:** The applied promotion can now be **removed** by clearing the promo code input and clicking "Apply," or by applying a new valid/invalid code. This fixes the issue where the promo code was permanent and could not be changed.
* **Multiple Promo Code Application:** Users can now immediately **apply new promo codes** (valid or invalid) after removing the previous one, fixing the issue that blocked further promo testing.
* **Newsletter Subscription Success:** The subscription form no longer routes the page to the top. It now shows a **success notification** (e.g., "Your email has been successfully added to our mailing list. You’ll start receiving our newsletters soon").
* **Item Deletion Confirmation:** A simple **confirmation dialog** has been implemented before deleting an item from the cart to prevent accidental removal.
* **Delivery Form Email Field:** *PENDING/WORKAROUND:* An email field **has not been added** to the current front-end form as this is dependent on the future backend implementation. **This test case remains blocked** until the full user account/checkout system is implemented.

---

## ⚠️ Troubleshooting and Known Issues 🐛

This section details issues we are aware of and provides guidance for debugging.

* **Known Issue 1: Contact Us Page is Static:** The form on `contact.html` is currently a placeholder and **does not send data**. This is planned for future backend integration.
    * *Workaround:* The page provides static contact details (email/phone) for external communication.
* **Known Issue 2: Cart Persistence in Different Browsers:** While the cart uses `localStorage` for persistence, this data is unique to the browser and device. Clearing the browser cache will reset the cart.
* **Known Issue 3: Phone Validation Format:** The current phone validation (see Demo below) is a front-end check using specific Philippine number formats (`09XXXXXXXXX` or `639XXXXXXXXX`). Other international formats will display an invalid error.
* **Known Issue 4: Missing Email Field in Delivery Form:** The delivery form does not currently include an email field, blocking email validation testing. This will be addressed with the future **Full User Account system** implementation.
...
 - **Verify Item Removal Confirmation:** Try deleting an item from the cart. Confirm that a confirmation dialog appears before the item is removed.
...
 - **Verification (Removal/Change):** To remove the promo: **clear the promo input and press Enter or click Apply while empty**; the discount is removed and totals update. You can then immediately apply a different valid or invalid code.
...
3) **Newsletter Subscription Verification**
 - Scroll to the footer section. Enter an email in the "Subscribe to our Newsletter" field.
 - Click **"Subscribe."** **Verification:** The page should **not** jump to the top. A success notification (e.g., "Your email has been successfully added...") should appear near the field.

4) **Checkout phone validation (Philippine mobile numbers)**
...
5) **Quantity and cart behaviours**
...
- **Implement functional Contact Us page and Delivery Form with email validation.**
